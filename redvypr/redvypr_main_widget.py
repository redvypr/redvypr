...
class redvyprWidget(QtWidgets.QWidget):
    ...
    def start_main_application(self):
        ...
        new_hostname = config_init.pop("hostname")

        # Initialize redvypr
        self.initialize_redvypr(new_hostname)

        # Add the metadata (if existing)
        metadata = {}
        extra_attributes = config_init.pop("extra_attributes")
        for k,content in config_init.items():
            if len(content) > 0:
                metadata[k] = content

        for k, content in extra_attributes.items():
            if len(content) > 0:
                metadata[k] = content
    ...
    def show_loglevelwidget(self):
        """Show the loglevelwidget

        """
        self.__loglevelwidget = redvypr.logging_utils.loglevelWidget(redvypr=self.redvypr)
