...
class Device(RedvyprDevice):
    ...
    def add_measurement_to_metadata(self, measurement):
        ...
        self.redvypr.set_metadata(meta_address.to_address_string(), metadata=metadata)
...
class RedvyprDeviceWidget_legacy(RedvyprdevicewidgetStartonly):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Map: { unique_id: current_tab_index }
        # Using UUID for measurements and full_name for contacts
        self.opened_tabs = {}
    ...
    def setup_dashboard(self):
        ...
        dash_layout = QtWidgets.QHBoxLayout(dashboard)

        # --- Contact Management ---
        c_group = QtWidgets.QGroupBox("Contact Management")
        c_lay = QtWidgets.QVBoxLayout(c_group)
        self.contact_list = QtWidgets.QListWidget()
        self.contact_list.doubleClicked.connect(
            lambda: self.open_contact_tab(new=False))
        ...
        c_lay.addWidget(self.contact_list)
        c_lay.addWidget(add_c_btn)
        dash_layout.addWidget(c_group)
        ...
        add_m_btn = QtWidgets.QPushButton(" Add Measurement")
        add_m_btn.setIcon(qtawesome.icon('fa5s.layer-group'))
        add_m_btn.clicked.connect(lambda: self.open_meas_tab(new=True))

        m_lay.addWidget(self.meas_list)
        m_lay.addWidget(add_m_btn)
        dash_layout.addWidget(m_group)

        self.tabs.addTab(dashboard, qtawesome.icon('fa5s.tachometer-alt'), "Dashboard")
    ...
    def open_meas_tab(self, new=False):
        ...
        # Use UUID as the stable identifier for the tab map
        identifier = f"meas_{config.uuid}"

        if identifier in self.opened_tabs:
            tab_idx = self.opened_tabs[identifier]
            self.tabs.setCurrentIndex(tab_idx)
            return

        editor = MeasurementConfigEditor(config=config, device=self.device)
        # Connect the update signal to our sync method
        editor.config_updated.connect(lambda c: self.sync_measurement(c, is_new=new))
        editor.request_close.connect(lambda: self.close_tab_by_widget(editor))

        tab_name = config.name if config.name else "Unnamed"
        idx = self.tabs.addTab(editor, qtawesome.icon('fa5s.edit'), tab_name)
        self.opened_tabs[identifier] = idx
        self.tabs.setCurrentIndex(idx)

    def sync_measurement(self, config_obj, is_new):
        """
        Updates the model and the UI components (Tab name and List).
        """
        if is_new:
            self.custom_config.measurements.append(config_obj)
        else:
            # Update the existing object in the list based on UUID
            for i, m in enumerate(self.custom_config.measurements):
                if m.uuid == config_obj.uuid:
                    self.custom_config.measurements[i] = config_obj
                    break

        # Update the Tab text if the name changed
        identifier = f"meas_{config_obj.uuid}"
        if identifier in self.opened_tabs:
            tab_idx = self.opened_tabs[identifier]
            self.tabs.setTabText(tab_idx, config_obj.name)
    ...
    def refresh_dashboard_lists(self):
        ...
        self.meas_list.clear()
        for m in self.custom_config.measurements:
            display_name = m.name if m.name else "Unnamed Measurement"
            self.meas_list.addItem(display_name)

    def close_tab(self, index):
        if index == 0: return  # Protect Dashboard tab
        ...
        # Shift remaining indices in map
        for k in self.opened_tabs:
            if self.opened_tabs[k] > index:
                self.opened_tabs[k] -= 1
