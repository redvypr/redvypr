...
class ConfigGroupWidget_old(QtWidgets.QGroupBox):
    def __init__(self, title: str, config_key: str, device_config: Any,
        ...
        # Initialer Status aus der Config
        is_checked = getattr(self.device_config, self.config_key)

        # 1. GroupBox Checkbar machen (Nutzt die Checkbox im Titel)
        self.setCheckable(True)
        self.setChecked(is_checked)
        ...
        for field_name, description in self.fields.items():
            ...
            input_widget.valueChanged.connect(
                lambda val, fn=field_name: self.on_field_changed(fn, val))
            self.fields_layout.addRow(f"{description}:", input_widget)

        self.main_layout.addWidget(self.fields_widget)
        self.setLayout(self.main_layout)

        # 3. Signal der GroupBox verbinden (toggled sendet direkt einen bool)
        self.toggled.connect(self.toggle_visibility)
    ...
    def toggle_visibility(self, on: bool):
        """Schaltet Sichtbarkeit um und updatet Config."""
        self.fields_widget.setVisible(on)
        setattr(self.device_config, self.config_key, on)
...
# Setup
class RedvyprDeviceWidget_old(RedvyprdevicewidgetSimple):
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        #self.config_widget = pydanticDeviceConfigWidget(self.device)

        self.rand_group = ConfigGroupWidget(
            title="Random Data",
            config_key="send_rand",
            device_config=self.device.custom_config,
            fields={
                "rand_freq_send": "Send Frequency",
                "rand_amp": "Amplitude",
        ...
        # --- Sine Config Group ---
        sine_fields = {
            "sine_freq": "Frequency of sine",
            "sine_amp": "Amplitude of sine",
            "sine_freq_send": "Send frequency of sine data",
        }
        self.sine_group = ConfigGroupWidget(
            title="Sine",
            config_key="send_sine",
            device_config=self.device.custom_config,
            fields=sine_fields,
        )
        ...
        # --- Fast Config Group ---
        fast_fields = {
            "fast_freq_single": "Frequency of fast single data",
            "fast_freq_merged_send": "Frequency of merged packets",
        }
        self.fast_group = ConfigGroupWidget(
            title="Fast",
            config_key="send_fast_single",  # or "send_fast_merged" if you prefer
            device_config=self.device.custom_config,
            fields=fast_fields,
        )

        # 4. Fast Merged Config Group
        self.fast_merged_group = ConfigGroupWidget(
            title="Fast Merged",
            config_key="send_fast_merged",
            device_config=self.device.custom_config,
            fields={
                "fast_freq_merged_send": "Packet Frequency",
            }
        )
        ...
        self.layout.addWidget(self.rand_group)
        self.layout.addWidget(self.sine_group)
        self.layout.addWidget(self.fast_group)
        self.layout.addWidget(self.fast_merged_group)
